...
class EntityManager
{
  ...
  public void draw(Graphics2D g, GraphicsConfiguration graphicsConfiguration)
  {
    ...
    DrawingManager drawingManager = new DrawingManager(this, graphicsConfiguration, g.getClipBounds().width / scale,
        g.getClipBounds().height / scale, g.getClipBounds().width, g.getClipBounds().height);
    for (Entity entity : entities)
    {
      ...
      if (boundingBox != null)
      {
        Graphics2D local = (Graphics2D) scaledScreen
            .create((int) (boundingBox.x - cameraOrigin.x), (int) (boundingBox.y - cameraOrigin.y),
  ...
  private void initializeKeyboardDict()
  {
    for (Field field : KeyEvent.class.getDeclaredFields())
    {
      ...
      {
      }//must not be a public field, so don't need it
    }
  }
